...
IMPORTANT: In Next.js 15, params are async and need to be awaited.
  ...
  <verify>
    ...
    2. Create a test job via API and poll status endpoint:
       ```bash
       # Start dev server first, then in another terminal:
       # Login to get session cookie, then:
       curl -X POST http://localhost:3000/api/admin/generate \
         -H "Content-Type: application/json" \
         -d '{"topic": "Test Topic", "categoryId": "YOUR_CATEGORY_ID"}' \
         --cookie "your-session-cookie"
       # Note the jobId returned
       # Poll the status:
       curl http://localhost:3000/api/admin/status/JOB_ID_HERE \
         --cookie "your-session-cookie"
       # Verify status transitions: pending -> generating -> validating -> completed (or failed)
       ```
  </verify>
  <done>
    ...
    - Job status transitions through pending -> generating -> validating -> completed/failed
  </done>
</task>
