...
must_haves:
  truths:
    - "Admin sees trending topics for selected category on generation page"
    - "Admin can click a trending topic to auto-fill the topic field"
    - "Admin can enter a custom topic instead of selecting a suggestion"
    - "Trending topics refresh when admin changes the category selection"
    - "Recently used topics are filtered out from suggestions"
  artifacts:
